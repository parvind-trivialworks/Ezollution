...
function GetSchedulingData() {
    schedulerTable = $('#tblScheduler').DataTable({
        ...
        "paging": false,
        "ajax": {
            url: "/SeaScheduling/GetSchedulingData",
            method: "POST",
            data: function (d) {
        ...
        "columns": [
            ...
            { "data": "sClientName", "orderable": false, },
            {
                ...
                render: function (data, type, row, meta) {
                    if (row.iSAction == 2 || row.iSAction == 3)
                        return data.sMBLNumber;
                    else
                        return "<a href='/SeaScheduling/checklist?iSchedulingId=" + data.iSchedulingId + "' target='_top'>" + data.sMBLNumber + "</a>";
                }
            },
            { "data": "sPOD", "orderable": false },
            { "data": "sFPOD", "orderable": false },
            { "data": "sVesselName", "orderable": false },
            { "data": "sContainerNumber", "orderable": false },
            { "data": "sShippingLine", "orderable": false },
            { "data": "sEDA", "orderable": false },
            { "data": null, "orderable": false, render: function (data, type, row, meta) { return "" + data.sCheckListSent + " / " + data.sCheckListApproved + ""; } },
            { "data": "sRecieveOn", "orderable": false },
            {
                "data": null, "orderable": false,
                render: function (data, type, row, meta) {
                    return "<button  class='btn btn-info btn-xs' href='' data-toggle='tooltip' data-placement='top' title='" + data.sRemarks +"'><i class='fa fa-info'></i></button>  &nbsp; <button class='btn btn-warning btn-xs' onclick='AddEditScheduling(" + data.iSchedulingId + ")'><i class='fa fa-edit'></i></button>";
                }
            }, 
...
$(document).on('change', '#iSAction', function () {
    if ($('#iSAction').val() == 1) {
        ...
        $("#sCheckListSent").selectpicker('refresh');
    }
});
